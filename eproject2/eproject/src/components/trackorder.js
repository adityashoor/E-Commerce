...
export const Trackorder = () => {
    ...
    var fetchallorder=()=>
    {
        fetch(`${API_URL}/api/fetchorderbyusername?username=${user.username}`).then(resp=>
        {
            if(resp.ok)
            {
...
{table===true?<>
    <div className='trackbody'>
        <div className="container-fluid d-sm-flex row">
            ...
            <div key={data._id || data.orderid} className="card px-2 mt-lg-5 mt-4 ms-sm-2 col-lg-3 col-10">
                    <div className="card-header bg-white">
                    <div className="row justify-content-between">
                        <div className="col">
                            <p className="text-muted"> Order ID  :<span className="font-weight-bold text-dark">{data.orderid}</span></p> 
                            <p className="text-muted"> Bill Amount <span className="font-weight-bold text-dark"></span>CA${data.totalcost}</p></div>
                    </div>
                    </div>
                    <div className="card-body">
                        <div className="media flex-column flex-sm-row">
                            <div className="media-body ">
                                ...
                                <h4 className="mt-3 mb-4 bold"> <span className="mt-5">CA$</span> {data.prodrate} <span className="small text-muted"> via (cod) </span></h4>
