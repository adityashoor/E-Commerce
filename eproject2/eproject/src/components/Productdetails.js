...
export const ProductDetails = () => {
  ...
  const [avalstock, setavalstock] = useState([]);
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/getprodbyproid/${prodid}`).then((resp) => {
      if (resp.ok) {
  ...
  {
    if(!stock) return;
    const limit = stock > 10 ? 10 : stock;
    const newstock = [];
    for (let i = 1; i <= limit; i++) {
        newstock.push(i);
    }
    setavalstock(newstock);
  },[stock])
...
var addprodtocart=async()=>
{
  ...
  try {
      const resp = await fetch(`${API_URL}/api/addtocart`,
          {
              method: "post",
  ...
  var addtocart=()=>
    {
        if(user)
        {
            fetch(`${API_URL}/api/searchcart?user=${user.username}&prodid=${prodid}`).then(resp=>
            {
              if(resp.ok)
              {
                resp.json().then(result=>
          ...
          <div className="container">
            <div className="w3breadcrumb-gids">
              <div className="w3breadcrumb-left text-left">
                <h2 className="inner-w3-title">{proddata.productname || "Product Details"}</h2>
              </div>
            ...
            <div className="sp-store-single-page row">
                ...
                <div className="flexslider1">
                  <div className="thumb-image">
                    <img
                      ...
                      alt={proddata.productname || "product"}
              ...
              <div className="col-lg-7 single-right-left ps-lg-5">
                ...
                <div className="caption">
                  <h6>
                    <span className="item_price">CA${remamt ? Math.round(remamt) : proddata.rate}</span>
                    {proddata.discount && Number(proddata.discount) > 0 && (
                      <del className="ms-2 text-muted">CA${proddata.rate}</del>
                    )} &nbsp; Free Delivery
                  </h6>
                </div>
