...
export const Cart = () => {
    ...
    useEffect(()=>
    {
        // Auto-remove items that have Qty <= 0 (edge case from minus going below 1)
        cartdata.forEach(item => {
            if(item.Qty <= 0) delcart(item._id);
        });

        const tcost = cartdata.reduce((sum, item) => sum + item.TotalCost, 0);
        setgtotal(tcost);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    },[cartdata])
                ...
                {table===true?<><div className="ecom-contenthny-w3lcheckout privacy">
                        ...
                        <table className="timetable_sub">
                            ...
                            <tbody>
                                ...
                                <tr key={data._id}>
                                    ...
                                    <td className="invert">
                                        <div className="quantity">
                                            <div className="quantity-select">
                                                ...
                                                <div className="entry value"><span>{data.Qty}</span></div>
                                    ...
                                    <td className="invert">CA${Math.floor(data.TotalCost)}</td>
                                    <td className="invert">
                                        <div className="rem">
                    ...
                    <div className="row checkout-left mt-5">
                        <div className="col-md-8 checkout-left-basket ms-auto ps-lg-5">
                           ...
                           <ul>
                            {cartdata.map((data)=>
                                <li key={data._id}>{data.ProdName} <i>-</i> <span>CA${data.TotalCost} </span></li>
                            )}
                                <li>Total Service Charges <i>-</i> <span>CA$10.00</span></li>
                                <li className='checkout-left-basketmain'>Total <i>-</i> <span>CA${gtotal+10}</span></li>
                            </ul>
