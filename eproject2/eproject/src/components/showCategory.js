            ...
            if (resp.ok) {
                resp.json().then(result => {
                    if (result.statuscode === 1) {
                        ...
                        toast.warning("No Categories Added");
                    } else {
                    ...
                    <div className="container">
                        <div className="w3breadcrumb-gids">
                            <div className="w3breadcrumb-left text-left">
                                <h2 className="inner-w3-title">Product Categories</h2>
                            </div>
                            <div className="w3breadcrumb-right">
                                <ul className="breadcrumbs-custom-path">
                                    <li><Link to="/homepage">Home</Link></li>
                                    <li className="active"><span className="fas fa-angle-double-right mx-2"></span>Categories</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <div className="container py-md-4">
                    ...
                    {allcat.length === 0 ? (
                        <div className="text-center py-5">
                            <i className="fa-solid fa-sitemap text-muted" style={{ fontSize: 48 }}></i>
                            <p className="text-muted mt-3">No categories available yet.</p>
                        </div>
                    ) : (
                        <div className="witemshny-grids row mt-lg-3">
                            {allcat.map(data => (
                                <div className="col-xl-2 col-md-4 col-6 product-incfhny mt-4" key={data._id}>
                                    <Link to={`/showsubcategory/${data._id}`}>
                                        <div className="weitemshny-grid">
                                            <img
                                                src={`/idata/${data.catpic}`}
                                                alt={data.catname}
                                        ...
                                        <h4 className="gdnhy-1 mt-3 text-center">{data.catname}</h4>
                                    </Link>
                                </div>
                            ))}
                        </div>
