...
// Save uploads to the React frontend's public/idata so images resolve
// correctly on the dev server (port 3000). Also copy to local public/idata.
const FRONTEND_IDATA = path.join(__dirname,'..','eproject2','eproject','public','idata');
const LOCAL_IDATA    = path.join(__dirname,'public','idata');

var picname;
let MyStorage=multer.diskStorage({
    destination:(req,file,cb)=>
    {
        cb(null, fs.existsSync(FRONTEND_IDATA) ? FRONTEND_IDATA : LOCAL_IDATA);
    },
...
mongoose.connect(process.env.MONGO_URI || "mongodb://127.0.0.1:27017/record",function(err)
{
    if(err)
    {
